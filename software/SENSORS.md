...
- ESP32 建立密码保护的 `QUAD-CONTROL` 热点。手机连接后访问 `http://192.168.4.1/`，在网页上发送四轴与解锁请求；默认密码见 [手机遥控说明](esp32_oled/README.md)。ESP32 将指令编码为 QCTRL v1，通过 GPIO17→F411 PA3 的 UART 发送。F411 接收中断校验 CRC，超过 250ms 未收到有效帧即判为失联。F411 已增加四路相同油门的拆桨台架输出，操作与电调行程校准见[电机说明](MOTORS.md)。
...
主循环仍有阻塞 I²C、串口发送和初始化延时，尚不是飞控实时控制调度。TIM3 中断独立维护 50Hz 电调脉冲并监测主循环是否停滞；代码没有姿态混控或闭环飞行控制。
