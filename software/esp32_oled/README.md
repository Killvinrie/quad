...
网页有两根虚拟摇杆：左杆左右控制 YAW、上下控制 THR；右杆左右控制 ROLL、上下控制 PITCH。点击“开始控制”后约每 50ms 发送一次原码 0～4095；松开左杆油门归零，松开右杆回中。油门归零且其他轴回中时，可点“请求解锁”；该按钮只发送请求，不显示 F411 的实际状态。再次点击停机。停止控制、切到后台或关闭页面后，网页发送停机请求并停止持续发送。ESP32 没有新指令时不再发 UART 控制帧，F411 在 250ms 后判链路失效。OLED 默认显示手机控制页，网页按钮可切换为 STM32 传感器页；传感器页仍由 F411 的 SW5 拨码选择。OLED 的手机页超时后不显示旧摇杆值。

F411 现在可输出四路相同油门的电调 PWM，尚无姿态混控或飞行级保护。请先看[电机拆桨联调及行程校准](../MOTORS.md)。手机网页和 Wi-Fi 链路先用于拆桨台架联调。
